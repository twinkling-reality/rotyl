...
export class PerceptionStore {
   ...
   *
   * A tracking run needs exactly the reading this store already pays for: 44 ms
   * a frame, and the same `SceneEmbedding` a click is answered from. Letting it
   * load its own would be a second vision encoder in memory and a second
   * twenty-megabyte download, for the same weights.
   *
