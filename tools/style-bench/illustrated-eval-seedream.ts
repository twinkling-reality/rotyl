// Leftover Fal spend on other edit families. publishReady stays false.
//
//   FAL_KEY=... node --experimental-strip-types --experimental-transform-types tools/style-bench/illustrated-eval-seedream.ts
...
const resultsPath = join(here, 'results-illustrated-eval-seedream.json');
const results: {
  schema: 1;
  publishReady: false;
  path: string;
  ...
  schema: 1,
  publishReady: false,
  path: 'Seedream 4.5 edit, then GPT Image 1.5 edit, keep-list',
  skipped: false,
...
const jobs = [
  { tag: 'seedream-edit', model: FAL_SEEDREAM_EDIT, listPriceUsd: 0.04 as const },
  { tag: 'gpt-edit', model: FAL_GPT_EDIT },
];
...
for (const job of jobs) {
  ...
  for (const still of stills) {
    ...
    try {
      ...
      console.log(`illustrated-eval-seedream: ${job.tag} ${still.id} reused`);
      continue;
    } catch {
      /* missing */
    }
    ...
    try {
      console.log(`illustrated-eval-seedream: ${job.tag} ${still.id}`);
      const bytes = await prepareStill(file);
      const prompt = keepPrompt(still.id);
      const images =
        job.model === FAL_SEEDREAM_EDIT
          ? await runFalSeedreamEdit({
      ...
      console.log(`illustrated-eval-seedream: ${job.tag} ${still.id} ${String(elapsedMs)}ms`);
    } catch (cause) {
      const elapsedMs = Date.now() - started;
      ...
      await writeFile(resultsPath, `${JSON.stringify(results, null, 2)}\n`);
      console.error(`illustrated-eval-seedream: ${job.tag} ${still.id} failed: ${error}`);
...
await writeFile(resultsPath, `${JSON.stringify(results, null, 2)}\n`);
console.log('illustrated-eval-seedream: wrote sheets. publishReady remains false.');
